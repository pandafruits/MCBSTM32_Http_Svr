        ...
        if (netHTTPs_LoginActive()) {
          if (passw[0] == 1) {
            ...
            netHTTPs_SetPassword (passw);
				...
				// if format is "yyyy-MM-ddThh:mm:ss" then set date time
				if(var[9] == '-' && var[12] == '-' && var[15] == 'T'
					&& var[18] == ':' && var[21] == ':')
				{
					RTC_SetDateTime(var+5);
				}
...
uint32_t netCGI_Script (const char *env, char *buf, uint32_t buflen, uint32_t *pcgi) {
  ...
  switch (env[0]) {
    ...
    case 't':
      ...
      // Environmental data from 'environment.cgx'
		  switch (env[2]) {
        case '1':
          len = sprintf (buf, &env[4], DateTimeNow);
          break;
        case '2':
          len = sprintf (buf, &env[4], CoreTemperature);
          break;
				case '3':
          len = sprintf (buf, &env[4], Ambient.Temp);
          break;
				case '4':
          len = sprintf (buf, &env[4], Ambient.Hum);
          break;
      }
